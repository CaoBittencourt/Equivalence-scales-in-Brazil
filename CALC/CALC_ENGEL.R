...
# 9. [mais ou menos, pouco pvalor] : POF 2002, todas as faixas etárias, sem sexo ------------------------------------------------------------
...
# 9. [mais ou menos para menos] sem per capita ------------------------------------------------------------
# POF2002, todas as faixas etárias, sem sexo
# Modelos 2SLS (para output de tabelas)
Map(
  pof = lista.pof2002_ss.engle.sample,
...
) %>% bind_rows(.) %>% View(.) #-> pof2002_ss.engle_scales
...
Map(
  ...
  function(
    ...
    lapply(
      n.child, 
      function(n){
        equivalence.scales.engel.rothbarth.econ_scale2(
          model = iv.engel.model,
          pessoa.referencia = pessoa_ref,
          na.h = 2, nc.h = n,
          na.r = 2, nc.r = 0
